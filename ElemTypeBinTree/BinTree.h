...
static int screen[40][80];//屏幕打印专用缓冲区

template<typename ElemType>
struct BinTree {
	void* data;//结点数据，采用模板类 
	BinTree *lchild, *rchild;//左右孩子指针 
	BinTree(ElemType d) {//构造函数 
		data = d;
		lchild = NULL;
		rchild = NULL;
	}
	...
	static BinTree* create(ElemType *str, int &index) {//由先序遍历字符串创建二叉树
		ElemType ch = str[index++];
		if (ch == '#') return NULL;
		BinTree *node = new BinTree<ElemType>(ch);
	...
	//由先序和中序重构二叉树 
	static BinTree * rebuildByPreIn(ElemType* PreOrder, ElemType *InOrder, int Len) {
		//如果P为空，立刻返回 
		if (Len == 0) return NULL;
		//取PreOrder的第一个元素作为根节点r 
		char r = PreOrder[0];
		BinTree *R = new BinTree<ElemType>(r);
	...
	//由后序和中序重构二叉树 
	static BinTree * rebuildByPostIn(ElemType* PostOder, ElemType *InOrder, int Len) {
		//如果P为空，返回 
		if (Len == 0) return NULL;
		//取PostOder的最后一个元素作为根节点r 
		char r = PostOder[Len - 1];
		BinTree *R = new BinTree<ElemType>(r); //查找r在InOrder中出现的位置pos 
		int pos = 0;
		while (pos < Len) {
			if (InOrder[pos] == r)
				break;
	...
	void flipRL() {
		BinTree *node = new BinTree<ElemType>(ElemType);//交换时用到的中间变量
	...
	ElemType maxNode(ElemType max) {//传入一个参数，调用的时候，该参数默认为根结点的data
